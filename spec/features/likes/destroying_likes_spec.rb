require 'rails_helper'
RSpec.describe 'displaying likes' do
  before do
    @user = create_user
    log_in @user
    @blog1 = @user.blogs.create(content: 'Oops')
    @blog2 = @user.blogs.create(content: 'I did it again')
    Like.create(user: @user, blog: @blog1)
  end
  it 'displays unlike button for blogs already liked' do
    visit '/blogs'
    expect(page).to have_text(@blog1.content)
    expect(page).to have_text('1 likes')
    expect(page).to have_button('Unlike')
    expect(page).to have_text(@blog2.content)
    expect(page).to have_text('0 likes')
    expect(page).to have_button('Like')
  end
  it 'deletes like and displays the changes in both profile and blogs page' do
    visit '/blogs'
    click_button 'Unlike'
    expect(current_path).to eq("/blogs")
    expect(page).not_to have_button('Unlike')
    expect(page).not_to have_text('1 likes')
    visit "/users/#{@user.id}"
    expect(page).not_to have_text('1 likes')
  end
  it 'creates like and displays it both in profile and blogs page' do
    visit '/blogs'
    click_button 'Like'
    expect(current_path).to eq('/blogs')
    expect(page).not_to have_button('Like')
    expect(page).not_to have_text('0 likes')
    visit "/users/#{@user.id}"
    expect(page).not_to have_text('0 likes')
  end
end